...
var controller = {
      ...
      var join = "";
        ...
        condition += "ims.uid = '" + selected_study + "' AND ";
			...
			if((typeof offset !== 'undefined' && offset !== '')){
				condition = " im.imaging_manifest_id > '" + offset + "' AND ";       
			}
      ...
      db.query(query,function(dataJson){
        rez = lowercaseObject(dataJson);
          ...
          ImagingManifest.id = rez[i].imaging_manifest_id;
					...
					ImagingManifest.author = rez[i].Author;
		...
		imagingManifestSeries: function getImagingManifestSeries(req, res) {
			...
			var imagingManifestStudyId = req.query.imaging_manifest_study_id;
		...
		imagingManifestInstance: function getImagingManifestInstance(req, res) {
			...
			var imagingManifestSeriesId = req.query.imaging_manifest_series_id;
		...
		imagingManifestSeriesEndpoint: function getImagingManifestSeriesEndpoint(req, res) {
			var apikey = req.params.apikey;
			
			var EndpointId = req.query._id;
			var imagingManifestId = req.query.series_id;
			...
			if (typeof imagingManifestId !== 'undefined' && imagingManifestId !== "") {
				condition += "IMAGING_MANIFEST_STUDY_SERIES_ID = '" + imagingManifestId + "' AND ";
			}
			...
			var arrImagingManifestSeriesEndpoint = [];
			var query = 'select endpoint_id from BACIRO_FHIR.endpoint ' + fixCondition;
			console.log(query);
			db.query(query, function (dataJson) {
				rez = lowercaseObject(dataJson);
				for (i = 0; i < rez.length; i++) {
					var imagingManifestSeriesEndpoint = {};
					if(rez[i].endpoint_id != "null"){
						imagingManifestSeriesEndpoint.id = hostFHIR + ':' + portFHIR + '/' + apikey + '/Endpoint?_id=' +  rez[i].endpoint_id;
					} else {
						imagingManifestSeriesEndpoint.id = "";
					}
					
					arrImagingManifestSeriesEndpoint[i] = imagingManifestSeriesEndpoint;
				}
				res.json({
					"err_code": 0,
					"data": arrImagingManifestSeriesEndpoint
				});
			}, function (e) {
				res.json({
					"err_code": 1,
					"err_msg": e,
					"application": "Api Phoenix",
					"function": "getImagingManifestSeriesEndpoint"
				});
			});
		},
		imagingManifestStudyEndpoint: function getImagingManifestStudyEndpoint(req, res) {
			var apikey = req.params.apikey;
			
			var EndpointId = req.query._id;
			var imagingManifestId = req.query.study_id;
			...
			if (typeof imagingManifestId !== 'undefined' && imagingManifestId !== "") {
				condition += "IMAGING_MANIFEST_STUDY_ID = '" + imagingManifestId + "' AND ";
			}
			...
			var arrImagingManifestStudyEndpoint = [];
			var query = 'select endpoint_id from BACIRO_FHIR.endpoint ' + fixCondition;
			console.log(query);
			db.query(query, function (dataJson) {
				rez = lowercaseObject(dataJson);
				for (i = 0; i < rez.length; i++) {
					var imagingManifestStudyEndpoint = {};
					if(rez[i].endpoint_id != "null"){
						imagingManifestStudyEndpoint.id = hostFHIR + ':' + portFHIR + '/' + apikey + '/Endpoint?_id=' +  rez[i].endpoint_id;
					} else {
						imagingManifestStudyEndpoint.id = "";
					}
					
					arrImagingManifestStudyEndpoint[i] = imagingManifestStudyEndpoint;
				}
				res.json({
					"err_code": 0,
					"data": arrImagingManifestStudyEndpoint
				});
			}, function (e) {
				res.json({
					"err_code": 1,
					"err_msg": e,
					"application": "Api Phoenix",
					"function": "getImagingManifestStudyEndpoint"
				});
			});
		},
  },
	...
	put: {
		imagingManifest: function updateImagingManifest(req, res) {
			console.log(req.body);
			var imaging_manifest_id  = req.params._id;
			var identifier  = req.body.identifier;
			var patient  = req.body.patient;
			var authoring_time  = req.body.authoring_time;
			var author_practitioner  = req.body.author_practitioner;
      ...
      var values = "";
        ...
        column += 'authoring_time,';
			...
			var condition = "imaging_manifest_id = '" + imaging_manifest_id + "'";
      ...
      var values = "";
        ...
        var condition = "study_id = '" + study_id+ "'";
      }
